...
def wdcnn():
    def wdcnn2(filters, kernerl_size, strides, conv_padding, pool_padding, BatchNormal):
        """wdcnn层神经元
        ...
        :return: model
        """
        model.add(Conv2D(filters=filters, kernel_size=kernerl_size, strides=strides,
                         padding=conv_padding, kernel_regularizer=l2(1e-4)))
        if BatchNormal:
            model.add(BatchNormalization())
        model.add(Activation('relu'))
        model.add(MaxPooling2D(padding=pool_padding))
        return model
    # 实例化序贯模型
    model = Sequential()
    # 搭建输入层，第一层卷积。因为要指定input_shape，所以单独放出来
    model.add(Conv2D(filters=16, kernel_size=(24, 24), strides=1, padding='same', kernel_regularizer=l2(1e-4),
                     input_shape=(32, 32, 1)))
    model.add(BatchNormalization())
    model.add(Activation('relu'))
    ...
    model = wdcnn2(filters=32, kernerl_size=(3,3), strides=1, conv_padding='same',
                  pool_padding='valid', BatchNormal=BatchNorm)
    # 第三层卷积
    model = wdcnn2(filters=64, kernerl_size=(3,3), strides=1, conv_padding='same',
                  pool_padding='valid',  BatchNormal=BatchNorm)
    # 第四层卷积
    model = wdcnn2(filters=64, kernerl_size=(3,3), strides=1, conv_padding='same',
                  pool_padding='valid', BatchNormal=BatchNorm)
